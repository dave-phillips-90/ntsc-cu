...
""",
    ...
    p.add_argument("-o", "--output", help="Output image file (default: <input>_ntsc.<ext>)")
    p.add_argument("--passes", type=int, default=1, help="Number of passes to apply (default: 1)")
    p.add_argument("--seed", type=int, default=None, help="Random seed for reproducible results")
    p.add_argument("--precise", action="store_true", help="Use precise (slower) emulation mode")
    p.add_argument("--resize", type=str, default=None, metavar="WxH",
                   help="Resize before processing (e.g. 640x480, 320x240). Result is scaled back to original size.")
    p.add_argument("--resize-width", type=int, default=None, metavar="W",
                   help="Resize width before processing (height auto-calculated to keep aspect ratio). Result is scaled back.")
    p.add_argument("--resize-height", type=int, default=None, metavar="H",
                   help="Resize height before processing (width auto-calculated to keep aspect ratio). Result is scaled back.")
    p.add_argument("--output-size", type=str, default=None, metavar="WxH",
    ...
    p.add_argument("--output-height", type=int, default=None, metavar="H",
                   help="Output height (width auto-calculated to keep aspect ratio).")
    p.add_argument("--crop", type=str, default=None, metavar="W:H",
                   help="Crop to aspect ratio before processing (e.g. 4:3, 16:9, 3:2). Center crop.")

    rand_group = p.add_argument_group("random preset")
    rand_group.add_argument("--random", action="store_true", help="Use randomized parameters (ignores other options)")

    comp = p.add_argument_group("composite signal")
    comp.add_argument("--composite-preemphasis", type=float, default=None, metavar="V", help="Composite preemphasis (0..8, default: 0)")
    comp.add_argument("--composite-preemphasis-cut", type=float, default=None, metavar="HZ", help="Preemphasis cutoff frequency (default: 1000000)")
    comp.add_argument("--chroma-lowpass-in", type=int, choices=[0, 1], default=None, help="Chroma lowpass before composite encode (0/1, default: 1)")
    comp.add_argument("--chroma-lowpass-out", type=int, choices=[0, 1], default=None, help="Chroma lowpass after composite decode (0/1, default: 1)")
    comp.add_argument("--chroma-lowpass-out-lite", type=int, choices=[0, 1], default=None, help="Use lighter chroma lowpass (0/1, default: 1)")

    noise = p.add_argument_group("noise")
    noise.add_argument("--video-noise", type=int, default=None, metavar="V", help="Video luma noise (0..4200, default: 2)")
    noise.add_argument("--chroma-noise", type=int, default=None, metavar="V", help="Chroma noise (0..16384, default: 0)")
    noise.add_argument("--chroma-phase-noise", type=int, default=None, metavar="V", help="Chroma phase noise (0..50, default: 0)")
    noise.add_argument("--chroma-loss", type=int, default=None, metavar="V", help="Chroma loss probability (0..100000, default: 0)")

    ring = p.add_argument_group("ringing")
    ring.add_argument("--ringing", type=float, default=None, metavar="V", help="Ringing amount (0.3..0.99, 1.0=off, default: 1.0)")
    ring.add_argument("--ringing2", action="store_true", default=None, help="Use alternative ringing algorithm")
    ring.add_argument("--ringing-power", type=int, default=None, metavar="V", help="Ringing power for ringing2 (2..7, default: 2)")
    ring.add_argument("--ringing-shift", type=float, default=None, metavar="V", help="Ringing shift for ringing2 (default: 0)")
    ring.add_argument("--freq-noise-size", type=float, default=None, metavar="V", help="Frequency noise size (0..0.99, default: 0)")
    ring.add_argument("--freq-noise-amplitude", type=float, default=None, metavar="V", help="Frequency noise amplitude (0..5, default: 2)")

    bleed = p.add_argument_group("color bleed")
    bleed.add_argument("--color-bleed-horiz", type=int, default=None, metavar="V", help="Horizontal color bleed (0..10, default: 0)")
    bleed.add_argument("--color-bleed-vert", type=int, default=None, metavar="V", help="Vertical color bleed (0..10, default: 0)")
    bleed.add_argument("--color-bleed-before", type=int, choices=[0, 1], default=None, help="Apply color bleed before other effects (0/1, default: 1)")

    sc = p.add_argument_group("subcarrier")
    sc.add_argument("--subcarrier-amplitude", type=int, default=None, metavar="V", help="Subcarrier amplitude (default: 50)")
    sc.add_argument("--subcarrier-amplitude-back", type=int, default=None, metavar="V", help="Subcarrier decode amplitude (default: 50)")
    sc.add_argument("--scanline-phase-shift", type=int, choices=[0, 90, 180, 270], default=None, help="Scanline phase shift (default: 180)")
    sc.add_argument("--scanline-phase-shift-offset", type=int, default=None, metavar="V", help="Scanline phase shift offset (0..3, default: 0)")

    vhs = p.add_argument_group("VHS emulation")
    vhs.add_argument("--vhs", action="store_true", help="Enable VHS emulation")
    vhs.add_argument("--tape-speed", choices=["SP", "LP", "EP"], default=None, help="VHS tape speed (default: SP)")
    vhs.add_argument("--vhs-sharpen", type=float, default=None, metavar="V", help="VHS output sharpening (1..5, default: 1.5)")
    vhs.add_argument("--vhs-edge-wave", type=int, default=None, metavar="V", help="VHS edge wave distortion (0..10, default: 0)")
    vhs.add_argument("--vhs-svideo", action="store_true", default=None, help="VHS S-Video output (skip composite re-encode)")
    vhs.add_argument("--vhs-no-chroma-vert-blend", action="store_true", default=None, help="Disable VHS chroma vertical blend")
    vhs.add_argument("--vhs-head-switching", action="store_true", default=None, help="Enable VHS head switching noise (needs height >= 486)")
...
def build_ntsc(args) -> Ntsc:
    ...
    if args.tape_speed is not None:
        ntsc._emulating_vhs = True
        ntsc._output_vhs_tape_speed = {"SP": VHSSpeed.VHS_SP, "LP": VHSSpeed.VHS_LP, "EP": VHSSpeed.VHS_EP}[args.tape_speed]
